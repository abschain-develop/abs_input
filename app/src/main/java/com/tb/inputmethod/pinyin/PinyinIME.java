    ...
    private void registerClipEvents() {
        ...
        manager.addPrimaryClipChangedListener(new ClipboardManager.OnPrimaryClipChangedListener() {
            ...
            public void onPrimaryClipChanged() {
                ...
                Log.d(TAG, "label:" + manager.getPrimaryClipDescription().getLabel());
                if (manager.hasPrimaryClip() && manager.getPrimaryClip().getItemCount() > 0
                        && !TextUtils.equals(manager.getPrimaryClipDescription().getLabel(), "abs_input_copy_data")) {

                    CharSequence addedText = manager.getPrimaryClip().getItemAt(0).getText();

                    if (addedText != null) {
                        Log.e(TAG, "isBase64:" + FormatUtil.isBase64(addedText.toString()));
                        if (!FormatUtil.isBase64(addedText.toString())) {
                            Log.e(TAG, "is not base 64 encode.");
                            return;
                        }
                        if (InputMethodApplication.getInstance().getLoginStatus() != LoginStatus.LOGGED) {
                            // 用户未登录
                            Utils.userNotLogin(PinyinIME.this);
                        } else {
//                            String currentAddress = InputMethodApplication.getInstance().getCurrentAddress();
//                            if (TextUtils.isEmpty(currentAddress)) {
//                                // 未设置正在通讯地址
//                                Utils.setCurrentAddress(PinyinIME.this);
//                            } else {
                            String decrypted = AlgorithmUtil.decrypt(addedText.toString());
                            Log.d(TAG, "decrypted:" + decrypted);
                            if (!TextUtils.isEmpty(decrypted)) {
                                // 解密成功
                                // 键盘未弹出时，则先弹出键盘
                                if (null == mSkbContainer || !mSkbContainer.isShown()) {
                                    Log.d(TAG, "mSkbContainer is not shown");
                                    showWindow(true);
                                }
                                if (null != mInputModeSwitcher)
                                    mInputModeSwitcher.switchModeForUserKey(-8);
                                if (null != mSkbContainer)
                                    mSkbContainer.updateInputMode(decrypted);
                            }
//                            }
//                        }
                        }
    ...
    private void chooseAndUpdate(int candId) {
        ...
        if (mDecInfo.getComposingStr().length() > 0) {
            ...
            Log.e(TAG, "chooseAndUpdate: " + resultStr);
            ...
            if (candId >= 0 && mDecInfo.canDoPrediction()) {
                ...
                if (ImeState.STATE_IDLE == mImeState) {
                    Log.e(TAG, "ImeState.STATE_IDLE == mImeState ：" + mDecInfo.getSplStrDecodedLen());
    ...
    public void responseSoftKeyEvent(SoftKey sKey) {
        ...
        if (sKey.isUserDefKey()) {
            if (sKey.getKeyCode() == -7) {
                ExtractedText extractedText = ic.getExtractedText(new ExtractedTextRequest(), 0);
                Log.d(TAG, "extracted text:" + extractedText.text.toString());
                // 无需要加密内容
                if (extractedText != null && !TextUtils.isEmpty(extractedText.text)) {
                    if (InputMethodApplication.getInstance().getLoginStatus() != LoginStatus.LOGGED) {
                        ...
                        Log.d(TAG, "is user def key -7");
                        String currentAddress = InputMethodApplication.getInstance().getCurrentAddress();
                        if (TextUtils.isEmpty(currentAddress)) {
                            // 未设置正在通讯地址
                            Utils.setCurrentAddress(this);
                        } else {
                            // 加密
                            SessionKey key = InputMethodApplication.getInstance().getSessionKey(currentAddress);
                            if (key != null) {
                                Log.d(TAG, "key is not empty");
                                byte[] bKey = Base64.decode(key.getKey(), Base64.NO_WRAP);
                                if (bKey != null) {
                                    String encrypted = AlgorithmUtil.encrypt(key.getVersion(), bKey, extractedText.text.toString());
                                    ic.deleteSurroundingText(extractedText.text.toString().length(), extractedText.text.toString().length());
                                    ic.commitText(encrypted, encrypted.length());
                                    Log.d(TAG, "encrypted:" + encrypted + "结束");
                                } else {
    ...
    private void showCandidateWindow(boolean showComposingView) {
        if (mEnvironment.needDebug()) {
            Log.d(TAG, "Candidates window is shown. Parent = "
                    + mCandidatesContainer + "，" + showComposingView);
        }
    ...
    /**
     * 每次切换输入焦点的时候，都会调用该方法，在这里可以完成和会话相关的初始化操作。
     * 比如前一个进程输入一半但未上屏的数据，切到另一个进程或输入控件后，就应该清除掉。该回调函数用来做这类工作。
     *
     * @param editorInfo
     * @param restarting
     */
                    ...
                    if (candId < 0) {
                        if (length() == 0) {
                            ...
                            for (int i = 0; i < length(); i++) {
                                ...
                                Log.e(TAG, "chooseDecodingCandidate: " + charAt(i));
                            }
                            mPyBuf[length()] = 0;

                            if (mPosDelSpl < 0) {
//                                Log.e(TAG, "chooseDecodingCandidate: "+mPyBuf.length+"-----"+length() );
                                //调用AIDL方法 AIDL方法里面调用JNI函数 通过键盘输入 给JNI函数传入一个输入字母的byte数组
                                // 数组会返回候选词的数量 根据候选词数量 循环 取对应位置的候选词  也是调用JNI 函数 调用JNI函数返回字符串
                ...
                //Log.e(TAG, "chooseDecodingCandidate: "+totalChoicesNum+"-----candid"+candId);
        ...
        private void updateDecInfoForSearch(int totalChoicesNum) {
            ...
            try {
                ...
                Log.e(TAG, "updateDecInfoForSearch: " + mFullSent);
        ...
        public void preparePredicts(CharSequence history) {
            ...
            if (Settings.getPrediction()) {
                ...
                if (null != preEdit) {
                    try {
                        Log.e(TAG, "preparePredicts: " + preEdit);
        ...
        public boolean pageForwardable(int currentPage) {
            Log.i(TAG, "mPageStart.size():" + mPageStart.size());

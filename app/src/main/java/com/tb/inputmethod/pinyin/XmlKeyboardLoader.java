...
public class XmlKeyboardLoader {
    private static final String THIS_FILE = "XmlKeyboardLoader";
    /**
...
//                        Log.d(THIS_FILE, "createKeyType:" + id + "," + bg);
...
//        Log.d(THIS_FILE, "getFloat:" + name);
        ...
        if (resId == 0) {
            ...
            try {
                float ret;
//                Log.d(THIS_FILE, "getFloat s:" + s);
                if (s.endsWith("%p")) {
                    ret = Float.parseFloat(s.substring(0, s.length() - 2)) / 100;
                } else if (s.endsWith("dp")) {
//                    Log.d(THIS_FILE, "getFloat ret：" + s.substring(0, s.length() - 2));
                    ret = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, Float.parseFloat(s.substring(0, s.length() - 2)), mContext.getResources().getDisplayMetrics());;
//                    Log.d(THIS_FILE, "getFloat ret：" + ret);
                } else {
                    ret = Float.parseFloat(s);
//                    ret = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, Float.parseFloat(s), mContext.getResources().getDisplayMetrics());;
                }
                return ret;
            } catch (NumberFormatException e) {
//                Log.d(THIS_FILE, "getFloat ret：" + e.getMessage());
                return defValue;
            }
        } else {

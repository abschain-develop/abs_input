...
public class SkbContainer extends RelativeLayout implements OnTouchListener {
    private static final String THIS_FILE = "SkbContainer";
    /**
    ...
    private View decryptLayout;
    private TextView decryptTv;
    private View decryptBtn;
...
//            mYBiasCorrection = Y_BIAS_CORRECTION;
    ...
    public void updateInputMode() {
        ...
        Log.d(THIS_FILE, "skbLayout:" + skbLayout);
    ...
    public void updateInputMode(String decrypted) {
        int skbLayout = mInputModeSwitcher.getSkbLayout();
        Log.d(THIS_FILE, "skbLayout:" + skbLayout);
        if (mSkbLayout != skbLayout) {
            mSkbLayout = skbLayout;
        ...
        if (null == skb) return;
        if (null == decryptTv) {
            decryptTv = findViewById(R.id.decrypt_tv);
        }
        if (decryptTv != null) {
            decryptTv.setText(decrypted);
        }
        skb.enableToggleStates(mInputModeSwitcher.getToggleStates());
        invalidate();
        return;
    ...
    private void updateSkbLayout() {
        ...
        if (null == decryptLayout) {
            decryptLayout = findViewById(R.id.decrypt_layout);
        }
        if (null == decryptBtn) {
            decryptBtn = findViewById(R.id.decrypt_close_btn);
            decryptBtn.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View view) {
                    if (null != mSkbFlipper) {
                        mSkbFlipper.setVisibility(View.VISIBLE);
                    }
                    if (null != decryptLayout) {
                        decryptLayout.setVisibility(View.INVISIBLE);
                    }
                    mInputModeSwitcher.switchModeForUserKey(-9);
                    updateInputMode();
                }
            });
        }
        ...
        switch (mSkbLayout) {
            case R.xml.skb_qwerty:
                if (mSkbFlipper.getVisibility() != View.VISIBLE)
                    mSkbFlipper.setVisibility(View.VISIBLE);
                if (decryptLayout.getVisibility() != View.INVISIBLE)
                    decryptLayout.setVisibility(View.INVISIBLE);
                Log.i(THIS_FILE, "updateSkbLayout skb_qwerty");
            ...
            case R.xml.skb_sym1:
                ...
                if (decryptLayout.getVisibility() != View.INVISIBLE)
                    decryptLayout.setVisibility(View.INVISIBLE);
                Log.i(THIS_FILE, "updateSkbLayout skb_sym1");
            ...
            case R.xml.skb_sym2:
                ...
                if (decryptLayout.getVisibility() != View.INVISIBLE)
                    decryptLayout.setVisibility(View.INVISIBLE);
                Log.i(THIS_FILE, "updateSkbLayout skb_sym2");
            ...
            case R.xml.skb_smiley:
                ...
                if (decryptLayout.getVisibility() != View.INVISIBLE)
                    decryptLayout.setVisibility(View.INVISIBLE);
                Log.i(THIS_FILE, "updateSkbLayout skb_smiley");
            ...
            case R.xml.skb_phone:
                ...
                if (decryptLayout.getVisibility() != View.INVISIBLE)
                    decryptLayout.setVisibility(View.INVISIBLE);
                Log.i(THIS_FILE, "updateSkbLayout skb_phone");
                majorSkb = skbPool.getSoftKeyboard(R.xml.skb_phone,
                        R.xml.skb_phone, screenWidth, skbHeight, mContext);
                break;
            case R.xml.skb_decrypted:
                if (mSkbFlipper.getVisibility() != View.INVISIBLE)
                    mSkbFlipper.setVisibility(View.INVISIBLE);
                if (decryptLayout.getVisibility() != View.VISIBLE)
                    decryptLayout.setVisibility(View.VISIBLE);
                Log.i(THIS_FILE, "updateSkbLayout skb_decrypted");
                majorSkb = skbPool.getSoftKeyboard(R.xml.skb_decrypted,
                        R.xml.skb_decrypted, screenWidth, skbHeight, mContext);
                break;
            default:
        }

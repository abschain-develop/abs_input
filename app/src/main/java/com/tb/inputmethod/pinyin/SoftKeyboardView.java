...
public class SoftKeyboardView extends View {
    private static final String THIS_FILE = "SoftKeyboardView";
    /**
...
//            Log.i(THIS_FILE, "setBackgroundDrawable");
//        }
        // 设置键盘背景色
        setBackgroundColor(getResources().getColor(R.color.keyboard_view_bg));
    ...
    private void drawSoftKey(Canvas canvas, SoftKey softKey, int keyXMargin,
        ...
        if (mKeyPressed && softKey == mSoftKeyDown) {
            Log.i(THIS_FILE, "mKeyPressed && softKey == mSoftKeyDown");
        ...
        if (null != bg) {
            bg.setBounds(softKey.mLeft + keyXMargin, softKey.mTop == 0 ? softKey.mTop + keyYMargin * 2 : softKey.mTop + keyYMargin,
                    softKey.mRight - keyXMargin, softKey.mBottom - keyYMargin);
            bg.draw(canvas);
            Log.i(THIS_FILE, "bg != null");
        }
        ...
        if (null != keyIcon) {
            ...
            float y = softKey.mTop + marginY - mFmi.top + mFmi.bottom / 1.5f + (softKey.mTop == 0 ? keyYMargin : keyYMargin / 2.0f);
